...
table "dhcprange" {
  ...
  column "dhcp_range_search_vector" {
    null = true
    type = tsvector
  }
  primary_key {
  ...
  index "ix_dhcp_range_search_vector" {
    columns = [column.dhcp_range_search_vector]
    type    = GIN
  }
  index "ix_dhcprange_dhcp_range_search_vector" {
    columns = [column.dhcp_range_search_vector]
    type    = GIN
  }
...
table "subnet" {
  ...
  index "ix_network_search_vector" {
    columns = [column.network_search_vector]
    type    = GIN
  }
  index "ix_subnet_id" {
    columns = [column.id]
  }
  index "ix_subnet_network_search_vector" {
    columns = [column.network_search_vector]
    type    = GIN
  }
}
